import {ADD_ITEM, MODIFY_ITEM, DELETE_ALL_ITEM, DELETE_THIS_ITEM, BUY_ITEMS} from "../actions/basketActions";
...
const basketReducer = (state = INITIAL_STATE , action) => {
    ...
    switch (action.type) {
        ...
        case DELETE_THIS_ITEM:
            newState = {
                basket: newState.basket.filter(product => product.pcode !== action.payload.pcode),
                count : newState.count -= 1
            }
            break;
        // 장바구니 상품 전체 삭제
