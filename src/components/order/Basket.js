...
function Basket({history}) {
    ...
    useEffect(()=> {
        ...
        let total = baskets.reduce((total, product, i)=>{
                        if(pcodeList.includes(product.pcode)) total+= product.price * quantity[i];
                         return total;
                     }, 0)
        setTotalPrice(total);
    ...
    let quantityButtonHandler = (index, product, number) =>{
        ...
        let total = baskets.reduce((total, product, i)=>{
            if(pcodeList.includes(product.pcode)) total+= product.price * q[i];
            return total;
            }, 0)
    ...
    //선택 상품들 장바구니 삭제 버튼
    let checkDelete = () =>{
        let pcodes = checkProduct.reduce((arr, product) => {
            arr.push(product.pcode);
            return arr;
            },[]);
